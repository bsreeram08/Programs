...
vector<int> stringAnagram(vector<string> dictionary, vector<string> query) {
    ...
    map<map<int,int>,int> mdictionary;
    int cnt;
    for(int iter=0;iter<dictionary.size();iter++){
        t.clear();
        for(int iter2=0;iter2<dictionary[iter].size();iter2++){
            t[dictionary[iter][iter2]]++;
        }
        mdictionary[t]++;
    }
    for(int iter=0;iter<query.size();iter++){
        cnt=0;
        compare.clear();
        for(int iter2=0;iter2<query[iter].size();iter2++){
            compare[query[iter][iter2]]++;
        }
        result.push_back(mdictionary[compare]);
    }
    return result;
}
int main()
